...
NFL_X = io.loadmat('NFL_Selected_X.mat')['MLHM2_X']
assert NFL_X.shape[0] == 36

NFL_X = xsacler.transform(NFL_X)
...
for e in pbar:
    ...
    for (datas, datat) in zip(source_trainloader, target_trainloader):
        ...
        src_gen_A = encoderA(src_data*torch.from_numpy(np.random.binomial(size=src_data.size(), n=1, p=1-noise)))# From A to B (HM->NFL)
        tgt_gen_B = encoderB(tgt_data*torch.from_numpy(np.random.binomial(size=tgt_data.size(), n=1, p=1-noise))) # From B to A (NFL->HM)

        tgt_gen_BA = encoderA(tgt_gen_B*torch.from_numpy(np.random.binomial(size=tgt_data.size(), n=1, p=1-noise)))  # Recovery: target->source->target (NFL->HM->NFL)
        src_gen_AB = encoderB(src_gen_A*torch.from_numpy(np.random.binomial(size=src_data.size(), n=1, p=1-noise)))  # Recovery: source->target->source (HM->NFL->HM)
...
# Save models
os.chdir(Dir_models)
specs = 'Noisy'+ str(noise)+'_Bi-GAN_HM2NFL_standardization_five layers_nonsaturating_separateGDE_dim' + str(dim) +'_epoch'+str(epochs) + '_G initial_lr' + str(lr) + '_D initial_lr' + str(lr3)+ '_Grec initial_lr' + str(lr2)+ '_beta' + str(beta) + '_alpha' + str(alpha)
torch.save({
    'epoch': epochs,
...
plt.yticks(fontsize=18)
plt.grid()
plt.legend(['Real HM Classification Accuracy','Fake HM Classification Accuracy','Real NFL Classification Accuracy','Fake NFL Classification Accuracy'],fontsize=18)
